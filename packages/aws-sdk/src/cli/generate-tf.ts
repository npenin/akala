...
nodeLinker: node-modules
    ...
    const dependencieszip = await new Promise<void>(async resolve =>
    {
        ...
        await walkTree(join(packagingFolder, 'container-deps', 'nodejs'), async (entry, folder, root) =>
        {
            ...
            await zipFile.add(zipFilePath, new zip.Uint8ArrayReader(await fs.readFile(join(entry.parentPath, entry.name))));
        });
        ...
        if (await hasYarn(join(packagingFolder, 'container-deps/nodejs/')))
        {
            try
            {
                return {
                    ...
                    output_hash: base64.base64EncArr(await crypto.subtle.digest('SHA-1', await fs.readFile(join(packagingFolder, 'container-deps/nodejs/node_modules/.yarn-state.yml'))))
                }
            }
            catch (e)
            {
                if (e.code == 'ENOENT')
                    return {
                        output_path: join(outputPath, 'container-deps.zip'),
                        output_hash: base64.base64EncArr(await crypto.subtle.digest('SHA-1', await fs.readFile(join(packagingFolder, 'container-deps/nodejs/package-lock.json'))))
                    }
                else
                    throw e;
            }
    ...
    }).then(async () =>
    {
        return {
            ...
            output_hash: base64.base64EncArr(await crypto.subtle.digest('SHA-256', await fs.readFile(join(outputPath, 'container.zip'))))
        }
    });
