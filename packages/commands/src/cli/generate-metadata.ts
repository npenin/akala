...
`)
    ...
    if (outputFile.endsWith('.d.ts'))
        await write(output, 'declare namespace ' + toCamelCase(options.name));
    else
    {
        await write(output, '// eslint-disable-next-line @typescript-eslint/no-namespace\n');
        await write(output, 'namespace ' + toCamelCase(options.name));
    }
    await write(output, '\n{\n');
    ...
    await write(output, '}\n');
    await write(output, '\n');
    await write(output, `export { ${toCamelCase(options.name)} as default };`);
