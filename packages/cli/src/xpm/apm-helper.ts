...
export default
    {
        ...
        {
            await spawnAsync(npm, { shell: true, cwd: path || process.cwd() }, 'setup', ...(options?.production ? ['--production'] : []));
        },
        async install(packageName: string, path?: string): Promise<void>
        {
            await spawnAsync(npm, { shell: true, cwd: path || process.cwd() }, 'add', packageName, '--prefix', path || process.cwd());
        },
        async uninstall(packageName: string, path?: string): Promise<void>
        {
            await spawnAsync(npm, { shell: true, cwd: path || process.cwd() }, 'del', packageName, '--prefix', path || process.cwd(), '--production');
        },
        async update(packageName: string, path?: string): Promise<void>
        {
            await spawnAsync(npm, { shell: true, cwd: path || process.cwd() }, 'up', packageName, '--prefix', path || process.cwd(), '--production');
        },
        ...
        async info(packageName: string, path?: string): Promise<packagejson.CoreProperties>
        {
            ...
            await spawnAsync(npm, { stdio: ['ignore', stdout, 'pipe'], shell: true, cwd: path || process.cwd() || process.cwd() }, 'info', packageName, '--json');
