...
(async function ()
{
    assert.deepStrictEqual(await program.process<Record<string, string>>({ abort: new AbortController(), args: ['a'], argv: ['a'], options: {}, currentWorkingDirectory: undefined, logger: null }), { opt: undefined })
    assert.deepStrictEqual(await program.process<Record<string, string>>({ abort: new AbortController(), args: ['a', 'x'], argv: ['a', 'x'], options: {}, currentWorkingDirectory: undefined, logger: null }), { opt: 'x' })
    console.log('ok');
})();
