...
program.command('client').command<{ name: string, url: string }>('api <name> <url>')
    .action(async function (context)
    {
        ...
        if (response.headers.has('content-type'))
            ...
            {
                program.process({ abort: new AbortController(), args: ['config', 'set', 'plugins.' + context.options.name, filePath + '/api.json'], argv: [], options: {}, currentWorkingDirectory: undefined, logger: context.logger });
                await promisify(fs.writeFile)(filePath + '/api.json', await response.text());
            }
            else
            {
                program.process({ abort: new AbortController(), args: ['config', 'set', 'plugins.' + context.options.name, filePath + '/api.js'], argv: [], options: {}, currentWorkingDirectory: undefined, logger: context.logger });
