...
export default async function init(this: State, pm: pmContainer, persistTo?: string)
{
    const queueProcessor = (msg: WorkflowInstance<object>, next: (processed: boolean) => void) =>
    {
        ...
        pm.dispatch('start', '@akala/automate/workflow', { abort: new AbortController(), options: { new: true, name: name, wait: true }, args: ['local'], argv: [], currentWorkingDirectory: process.cwd(), logger: null }).then(async () =>
        {
